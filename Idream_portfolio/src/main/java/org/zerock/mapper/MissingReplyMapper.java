...
public interface MissingReplyMapper {

	// 외래키를 사용하는 등록(create) 작업을 처리하는 insert() 추상 메서드 정의
	public int insert(MissingReplyVO vo);
	
	// 특정 댓글 읽기 기능의 조회(read) 작업을 처리하는 read() 추상 메서드 정의
	public MissingReplyVO read(Long rno);
	...
	public int update(MissingReplyVO reply);
	...
	// 구성합니다.
	public List<MissingReplyVO> getListWithPaging(
