...
public class NoticeReplyController {
	...
	public ResponseEntity<NoticeReplyPageDTO> getList(@PathVariable("page") int page, @PathVariable("bno") Long bno){
...
// STS에서 Console창에 처리된 메시지를 확인합니다.
	...
	public ResponseEntity<String> remove(@RequestBody NoticeReplyVO vo, @PathVariable("rno") Long rno){
	...
	@RequestMapping(method = {RequestMethod.PUT, RequestMethod.PATCH}, value = "/request/{rno}", consumes = "application/json", produces = {
			MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> modify(@RequestBody NoticeReplyVO vo, @PathVariable("rno") Long rno){
		vo.setRno(rno);
		log.info("rno : ▼-▼/ : " + rno);
		log.info("modify : ＆-＆/ : " + vo);
