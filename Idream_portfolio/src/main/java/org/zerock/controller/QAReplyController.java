...
import org.zerock.service.QAReplyService;
...
public class QAReplyController {
	...
	private QAReplyService service;
	...
	// 이때, '/{qno}/{page}'의 'page'값은 Criteria를 생성해서 직접 처리해 줍니다.
	// 즉, 게시물의 번호(qno)는 @PathVariable을 이용해서 파라미터로 처리해 줍니다.
	// 웹브라우저에 http://localhost:9005/replies/pages/720997/1 를 입력해서
	// 확인해 봅니다. 이때, 720997은 게시물의 번호(qno) 변수값이고, 1은 페이지(page)의 변수값입니다.
	public ResponseEntity<QAReplyPageDTO> getList(@PathVariable("page") int page, @PathVariable("qno") Long qno){
...
// 댓글 삭제 처리 : 데이터베이스에서 기존 게시물의 번호(qno)를 조회하고,
...
// STS에서 Console창에 처리된 메시지를 확인합니다.
	...
	// YARC에서 Payload 입력할 때 다음의 예시처럼 입력할 때, qno, replyer는 변하지 않습니다.
	// 왜냐하면, ReplyMapper.xml 에서 update 메서드 SQL 쿼리문을 보시면,
	// reply만 수정되게 되어 있기 때문입니다.
	// [다음 예시]
	// {"qno":720997, "reply":"댓글 수정을 확인합니다~ 오늘도 화이팅입니다~!", "replyer":"replyer9"}
	...
	@RequestMapping(method = {RequestMethod.PUT, RequestMethod.PATCH}, value = "/request/{rno}", consumes = "application/json", produces = {
			MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> modify(@RequestBody QAReplyVO vo, @PathVariable("rno") Long rno){
		vo.setRno(rno);
		log.info("rno : ▼-▼/ : " + rno);
		log.info("modify : ＆-＆/ : " + vo);
